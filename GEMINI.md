...
* **Parallel Extension Tracks (The HA Exception):** Since **SLE HA** and **SLES Core** are independent tracks, SLES for SAP does *not* automatically inherit SLE HA release notes (even though SLES for SAP includes HA packages). For HA-specific notes that appear in both SLE HA 16.x and SLES for SAP 16.x, using `shared.adoc` with tags is the preferred way to avoid source code duplication.
