==== Important Notes on Upgrading Slurm from a Previous Version
...
Slurm can be upgraded from version 22.05 to version 23.02 without loss
...
* Make it so `scrontab` prints client-side the `job_submit()` error messsage (which can
...
* The `--uid` option for the `srun` command is broken: its use may lead to the error message `job <job ID> queued and waiting for resources`.
This does not, however, affect the `sbatch` command.
