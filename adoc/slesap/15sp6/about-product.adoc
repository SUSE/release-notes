...
* FIPS 140-3 validation, see http://csrc.nist.gov/groups/STM/cmvp/documents/140-1/140InProcess.pdf
...
* Certain packages shipped as part of the module _Workstation Extension_ are L2-supported only
* Packages with names ending in `-devel` (containing header files and similar developer resources) are only supported together with their main packages.

{suse} only supports the usage of original packages. That is, packages that are unchanged and not recompiled.
