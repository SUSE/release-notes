...
The RPM `SAPHanaSR-ScaleOut` now contains an HANA HADR provider hook script for the method `preTakeover()`.
...
The current version of `SAPHanaSR-angi` 1.001 is targeting {hana} SR scale-up setups.
Nevertheless, CIB attributes are not backward compatible between `SAPHanaSR-angi` and `SAPHanaSR`. So there is currently no easy migration path.
