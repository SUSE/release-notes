...
The `crm` utility now provides the `crm cluster health sbd` command to analyze SBD configuration parameters.
This command detects misconfigurations or parameter imbalances that can occur after initial cluster bootstrap.
The command includes the `--fix` option to automatically resolve the detected misconfigurations.
