...
// bsc#1244260
[NOTE]
The algorithm control method also changed.
For more information see https://www.suse.com/support/kb/doc/?id=000021819.
...
// also jsc-PED-8205
[#bsc-1220091]
==== OpenSSL 3.1.4 is now default

In {productnameshort} {this-version}, OpenSSL has been updated to version 3.1.4, replacing OpenSSL 1.1.1.
...
Non-unified OVMF images for SEV (ovmf-x86_64-sev-{code,vars}.bin) are included for backward compatibility but present a
security risk  due to the non-measured variable store. For enhanced security with SEV, it is strongly recommended
to use the unified image (ovmf-x86_64-sev.bin).
The non-unified image is deprecated and will be removed in the next service pack.
