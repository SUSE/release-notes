...
However, it is possible to enable it permanently by editing the `/etc/zypp/zypp.conf` file by adding the following text:
...
There is new `zypper` HTTP backend that enables a new asynchronous downloader.
While it currently will not have massive impact on performance due to the
...
This feature can be enabled by setting the environmental variable `ZYPP_MEDIANETWORK` to `1`.
Because this feature is offered as a technology preview, enabling it system-wide is known to have issues, thus we recommend enabling this feature per command, for example:

[source,bash]
env ZYPP_MEDIANETWORK=1 zypper dup
...
[#jsc-SLE-15406]
===== 64K page size kernel flavor is available
...
As a technology preview, {arm-product} {previous-version} added a kernel flavor
...
====
{arm-product} {this-version} newly allows the use of Btrfs based file systems
...
[#jsc-SLE-21308]
===== LUKS2 support in the installer

LUKS2 is supported in the {yast} Partitioner as a tech preview.
This means that currently it has to be explicitly enabled.
