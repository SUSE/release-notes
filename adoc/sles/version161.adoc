...
=== Installer support of pervasive encryption
The storage JSON configuration supports pervasive encryption options (APQNs and key type). Updates the schema, adds JSON conversions for encryption properties, and adapts the encryption planner to properly assign APQN objects and key type to planned encrypted devices.

// jsc#PED-14602, 14610,14684,14872
=== Tooling supports z17 hardware
IBM libraries like libzdnn, libzpc, libica have been updated to support the latest hardware z17. Also development tools (glibc, valgrind) supports z17 now.
