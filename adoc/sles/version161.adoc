...
=== Confidential Computing Remote and Local Attestation (technology preview)

As a technology preview, remote and local attestation for AMD and Intel is available with the following packages in {productname} {this-version}:
...
// jsc#PED-15447
include::../shared.adoc[tags=jscped-15447,leveloffset=+2]
...
[#jsc-PED-14649]
=== Agama installer supports both standard and immutable installation modes

The Agama installer supports both standard and immutable installation modes.
Standard mode installs the system with a traditional read-write root directory.
Immutable mode installs the system with a read-only root directory and enables transactional updates.
The Agama configuration profile supports specifying the installation mode.
The installation mode cannot be changed after the installation is complete.
...
include::../shared.adoc[tags=jscped-16391,leveloffset=+2]
...
Information in this section applies to {arm-productname} {this-version}.
...
[#bsc-1281250]
=== Boot fails when PReP partition and root filesystem use different storage devices

On PowerVM, configuring the PReP boot partition and the root file system on different storage subsystems causes boot failures.
During POST, the system firmware initializes only the boot storage subsystem, which prevents GRUB from accessing secondary devices such as SAN multipath storage.
...
If you install {productname} {this-version} with the GNOME desktop on LPAR and try to login via the HMC virtual terminal, the login may time out while entering your credentials.
...
==== AMD SEV-SNP packages update

AMD SEV-SNP is supported in {productname} {this-version}.
The following AMD Secure Encrypted Virtualization (SEV-SNP) tools have been updated:

* `sevctl` has been updated to version 0.6.2.
* `snpguest` has been updated to version 0.10.0.
For changes, see https://github.com/virtee/snpguest/compare/v0.9.1...v0.10.0.
* `snphost` has been updated to version 0.7.0.

For details on AMD remote and local attestation, see <<jsc-PED-15646-tech-preview>>.
...
For details on Intel TDX remote and local attestation, see <<jsc-PED-15646-tech-preview>>.
...
// jsc#PED-16945
* {productname} 16.1 removes the `python-python-jose` and `python-python-jose-cryptography` packages.
Upstream does not maintain these packages.
You can install them from {ph}.
As an alternative, use the `python-PyJWT` package.
...
// jsc#PED-16552
// jsc#PED-16549
* These packages have been moved to {ph}:
** `fwts`
