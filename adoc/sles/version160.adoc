...
include::../shared.adoc[tags=jscped-12923,leveloffset=+2]
...
[#jsc-PED-12454]
=== New `kdump` configuration utility and options

The `kdumptool` utility has been extended with the `commandline` subcommand to manage `kdump`-related kernel command line options (such as `crashkernel=` and `fadump=`).
The subcommand accepts the following options:

* `-c`: Check if the current options found in `/proc/cmdline` are equal to the expected values.
* `-u`: Update the bootloader configuration with the expected values.
* `-d`: Delete `kdump`-related options from the bootloader configuration.

Additionally, two new configuration options are introduced in `/etc/sysconfig/kdump`:

* `KDUMP_CRASHKERNEL`: Specifies the `crashkernel` values, defaulting to `auto` (which calculates the required reservation size automatically).
* `KDUMP_UPDATE_BOOTLOADER`: Controls whether the bootloader configuration is updated automatically, defaulting to `true`.

By default, enabling the `kdump` systemd service automatically updates the bootloader on the first boot, making `kdump` fully functional on the second boot.
To enable `kdump` immediately on the next boot, manually execute `kdumptool commandline -u` after enabling the service.
...
[#jsc-PED-12534]
=== Simpler and more secure building of containers

Previously, Podman and Docker on SUSE Linux Enterprise were configured and patched to automatically inject the `SCCcredentials` file from the host into containers at both build time and runtime.
In {productname} {this-version}, this downstream-only patching has been removed.
The upstream-documented method of injecting secrets at build time only is now required.
This change also enables the shipping of the `buildx` backend for Docker.

To inject secrets at build time, use a specific `RUN` statement in the Dockerfile:

[source,dockerfile]
----
# Use the registration secret from the host to install a SLES-specific package
RUN --mount=type=secret,id=SCCcredentials zypper -n install kernel-default
----

Pass the secret on the command line when building:
...
[#jsc-PED-13243]
=== Secure Boot key size increased to 3072-bit RSA

The Secure Boot key has been upgraded from a 2048-bit to a 3072-bit RSA key.
This change ensures compliance with current Common Criteria (CC) and NIAP PP_OS requirements.
...
[#jsc-PED-12961]
==== Insecure NTLM authentication support removed

NTLM is no longer a secure authentication protocol because it uses MD4 and single-DES.
MD4 has been broken, and single-DES has too small a key size to be considered secure against brute-force attacks.
Support for NTLM authentication has been disabled where possible.
This change affects packages such as `libgsasl` and `curl` (except for `curl-mini`) (jsc#PED-12961).
