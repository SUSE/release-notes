...
The full list of changed packages compared to {previous-version} can be seen at this URL:

https://documentation.suse.com/package-lists/sle/{this-version}/package-changes_SLE-{previous-version-no-spaces}-GA_SLE-{this-version}-GA.txt

The full list of changed modules compared to {previous-version} can be seen at this URL:

https://documentation.suse.com/package-lists/sle/{this-version}/module-changes_SLE-{previous-version-no-spaces}-GA_SLE-{this-version}-GA.txt
...
SELinux replaces AppArmor, and is now set to enforcing by default.
For more informationm, see the _What is the status of SELinux?_ chapter of https://documentation.suse.com/sles/16.0//html/SLE-differences-faq/index.html#sle16-differences-faq-security-selinux[Key Technical Differences Between SLE 15 and SLE 16].
...
include::../shared.adoc[tag=jscped-11998,leveloffset=+2]
...
Now it needs to point to a loop-mounted ISO via FTP.
For example:
...
* {yast} has been removed. See <<bsc-1237605>> for more info.
