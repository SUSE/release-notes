...
* *Boot Parameters:* The legacy `install=` boot parameter is no longer supported.
+
See <<bsc-1261224>>.

=== During upgrade

* *Installer Requirements:* Running a virtual image on QEMU requires the `-cpu host` argument due to x86-64-v2 microarchitecture requirements.
+
See <<bsc-1240945>>.

=== After upgrading

* *Root Login:* Remote password-based root login is disabled by default.
+
Configure an SSH key for the root account during installation to enable the `sshd` service.
+
See <<bsc-1240889>>.

* *Service Configuration Upgrades:*
  ** *Dovecot 2.4:* Dovecot has been upgraded to version 2.4.
+
Review and update configurations manually.
+
See <<jsc-DOCTEAM-1714>>.
  ** *cgroup v2:* Only cgroups v2 are supported by default.
+
See <<jsc-PED-6311>>.
...
// jsc#PED-12658
* `rasdaemon` has been updated to version 0.8.3. This version supports machine checks related to CXL memory. Since Intel® Optane™ Memory products have reached End of Life (EOL), {productname} {this-version} does not support them either.
// jsc#PED-12283
...
A new pre-configured {productname} cloud image for Apache Tomcat is now available in cloud marketplaces.
This image provides a ready-to-use environment with Apache Tomcat and the OpenJDK runtime pre-installed and pre-configured.
It simplifies the deployment of Java-based web applications in public cloud environments.
For more information about deploying and configuring the image, see the _Apache Tomcat on SLES Cloud Guide_ at link:https://documentation.suse.com/sles/16.0/html/SLES-apache-tomcat-cloud/[].
...
// jsc#PED-12879
include::../shared.adoc[tags=jscped-12879,leveloffset=+2]
...
// bsc#1245474
[#jsc-PED-13286]
=== Network teaming (`teamd`) removed in favor of NetworkManager bonding

Network teaming, provided by the `teamd`, `libteam-tools`, and `NetworkManager-team` packages, has been removed and is no longer supported.
This change aligns with upstream trends, as the upstream `libteam` project has been inactive for several years.

To configure link aggregation such as Link Aggregation Control Protocol (LACP/IEEE 802.3ad) in SLES 16, use NetworkManager's built-in bonding capabilities instead.
NetworkManager bonding provides equivalent performance, load balancing, and failover capabilities.

For more information and detailed configuration instructions, see the https://documentation.suse.com/sles/16.0/html/SLES-networkmanager-nmcli/index.html#migrate-teamd[NetworkManager Guide].

[#jsc-PED-12110]
=== Centralized kernel hardening activation via `tuned` profile

Kernel hardening configurations accessible through the kernel command line have been enabled.
To mitigate the risk of performance degradation, their default state is disabled wherever feasible.
This establishes a baseline performance level while providing an option to activate enhanced security measures.

A dedicated `tuned-profiles-security` package provides a `tuned-hardening` profile.
This profile serves as a centralized mechanism to enable all kernel hardening features simultaneously, eliminating the need to manually configure each option.
It also simplifies testing and benchmarking of the performance impact of these hardening measures.

Enabling this profile may result in performance degradation in certain use cases, such as network workloads.

[#bsc-1261224]
=== Boot parameter changes

The `install=` boot parameter is no longer supported.
Use `inst.install_url=` and `root=live:` instead.
This change is especially important for netbooting, for example on {ppc64le} and {ibmz} systems.

For more information, see https://documentation.suse.com/sles/16.0/html/SLES-PXE-server/index.html#sles-pxe-server-backward-compatibility-with-sle-15[Backward Compatibility with SLE 15].

// jsc#PED-13633
include::../shared.adoc[tags=jscped-13633,leveloffset=+2]
...
{sap} systems using central user management with Microsoft Active Directory (as can be case for {s4hana} or for {hana}) require specific legacy library versions to function.
...
Microsoft SQL Server 2025 requires legacy hashing algorithms (such as MD2, MD4, and MD5) for functions like `HASHBYTE`.
To support this, OpenSSL 3 in {productnameshort} is now compiled with the `enable-md2` option.
Because these algorithms are disabled by default, the OpenSSL legacy provider must be explicitly enabled.

To enable the legacy provider, make the following modifications to `/etc/ssl/openssl.cnf`:

. In the `[provider_sect]` section, ensure that the `legacy` provider is defined:
+
[source,ini]
----
[provider_sect]
...
. Ensure that both default and legacy sections are activated:
+
[source,ini]
----
[default_sect]
...
To verify the providers are active, run the `openssl list -providers` command.
The output must display both `default` and `legacy` as active.

include::../shared.adoc[tag=jscped-15968,leveloffset=+3]
...
[#jsc-SLE-23244]
===== KubeVirt support for AMD SEV and SEV-ES

We now provide AMD Secure Encrypted Virtualization (SEV) and Secure Encrypted Virtualization-Encrypted State (SEV-ES) support for {kubevirt}.
This support allows you to secure guest virtual machines with memory encryption.
You can now run encrypted container-native virtualization workloads.
...
// jsc#PED-13286
// bsc#1245474
* Network teaming has been removed.
This includes the `teamd`, `libteam-tools`, and `NetworkManager-team` packages.
Use NetworkManager's built-in bonding capabilities instead.
For more information, see <<jsc-PED-13286>>.

// jsc#PED-15523
[#jsc-PED-15523]
* Prometheus has been removed.
It was added by accident to {productname} 16.0 and is unsupported.
...
// jsc#PED-16945
* The `python-python-jose` and `python-python-jose-cryptography` packages are unsupported and deprecated in {productname} 16.0.
Upstream does not maintain these packages.
{productname} 16.1 does not include these packages.
