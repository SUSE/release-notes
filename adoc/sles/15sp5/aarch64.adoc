...
* {amperereg} {xgenereg}, {emagreg}, {altrareg}, _{altramax}_, _**{ampereonereg}**_
...
[#jsc-PED-1763]
==== Driver enablement for {nvidiaorin}
...
// TODO NVIDIA IGX Orin

// bsc#1212541, jsc#PED-7863
[WARNING]
.Firmware/kernel ABI break in {nvidia} {jetpack} 6.0 SDK
====
...
A {arm-product} {this-version} kernel maintenance update adopts the behavior of
the latest kernels and is _fully_ compatible only with {jetpack} 6.0 and later:

|===
|Firmware |{slsa} {this-version} GM |{slsa} {this-version} QU1 |{slsa} {this-version} QU2 |{slsa} {this-version} QU3
...
* {nvidia} {jetson} AGX{nbsp}{orin} Developer Kit:
** USB Type-C controller
** Ethernet controller
** Camera sensors
** PCIe endpoint mode
* {nvidia} {jetson} {orin}{nbsp}NX:
** Camera sensors
* {nvidia} {jetson} {orin}{nbsp}Nano:
** Camera sensors
* For others, check with your system vendor.

For installed systems running {jetpack} firmware versions prior to 6.0,
suggested steps are:

. Update the kernel packages from the installed system,
  but do not reboot into the new kernel yet.
. Shut the system down in order to enter its Recovery Mode.
. Flash the latest {jetpack} firmware according to system vendor instructions.
  Be careful to not overwrite your {arm-product} installation.
...
* {arm-product} {this-version} Quarterly Update 3 (QU3) media from {scc}
are expected to include the updated kernel suited for systems with
{nvidia} {jetpack} 6.0 and later.
Flash the latest vendor firmware before installing from these updated media.
* In the meantime, with {this-version} Quarterly Update 2 and earlier media,
consider installing from an earlier version of {jetpack} and following above
update steps.
* If a firmware downgrade is not available for your system,
contact {suse} Customer Support for whether a custom kISO medium can be created.

{arm-product} {next-version} will only support {nvidia} {jetpack} 6.0 and later.
====

Drivers for the integrated, {nvidia} _Ampere_ microarchitecture-based Graphics Processor Unit (GPU) are not included (<<#jsc-PED-1763-drm>>).


[#jsc-PED-4564]
==== Driver enablement for {nvidiagrace}

A {arm-product} {previous-version} maintenance update added partial
enablement for the {nvidiagracereg} SoC (T241).

{arm-product} {this-version} adds further enablement for the {nvidiagrace} SoC.

This also covers the {nvidiagracehopperreg} SoC with an integrated,
_Hopper_ microarchitecture-based Graphics Processor Unit (GPU).
Drivers for this integrated GPU are not included (<<jsc-PED-4564-drm>>).

{nvidia} recommend to use `kernel-64kb` on {nvidiagrace} and {gracehopper} SoCs
(<<jsc-PED-4489>>).
...
{arm-product} {this-version} largely removes this technology preview status,
offering support for `kernel-64kb` on select platforms, such as
{nvidiagracereg} (<<jsc-PED-4564>>).
...
====
{arm-product} {previous-version} and later allow the use of Btrfs based file systems
